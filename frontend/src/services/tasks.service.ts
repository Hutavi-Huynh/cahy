...
export const tasksService = {
  getAll: (params?: { status?: TaskStatus; departmentId?: number; deadlineFrom?: string; deadlineTo?: string }) =>
  ...
  update: (id: number, data: Partial<Task>) => apiPatch<Task>(`/tasks/${id}`, data),
  ...
  getResults: (taskId: number) => apiGet<TaskResult[]>(`/tasks/${taskId}/results`),

  uploadFile: async (taskId: number, file: File): Promise<{ url: string; originalName: string }> => {
